...
def main():
    ...
    # First, let's get the 1B.1 numbers (without clear_undercut_periphery)
    tv_base, tf_base, tinfo_base = cg.trim_to_arch(verts, faces, frame, margin_mm=cg.ARCH_TRIM_MARGIN_MM)
    cv_base, cf_base, base_info_base = cg.build_cast_base(tv_base, tf_base, frame, rim=None)
    ...
    n_wall = len(tinfo_base["rim_loop"]) * 2
    ...
    for idx, (a, b) in enumerate(scan_wall_pairs[:8]):
        ...
        print(f"  Pokes past wall: ~{poke_mm} mm")
        ...
        blob = cg.write_binary_stl_bytes(rv, crop_f)
